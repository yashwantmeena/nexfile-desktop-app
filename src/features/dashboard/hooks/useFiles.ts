...
export function useFiles(mediaType?: string, query = "", searchMode = "tags", tags: string[] = [], refreshKey = 0, collection?: string, favoriteOnly = false) {
  ...
  const load = useCallback(async (offset: number, reset = false, clear = reset) => {
    ...
    setError(null);
    try {
      const page = await fetchFiles(offset, 60, mediaType, query, searchMode, JSON.parse(tagsKey), collection, favoriteOnly);
  ...
  }, [mediaType, query, searchMode, tagsKey, collection, favoriteOnly]);
  useEffect(() => {
  ...
  return { files, nextOffset, issues, loading, error, loadMore, updateFavorite };
}
