...
function formatCapturedAt(value?:number|null){return value == null ? "Unavailable" : new Date(value).toLocaleString();}
    ...
    <section className={`file-preview-modal ${infoOpen?"preview-info-open":"preview-info-collapsed"}`}>
      ...
      {infoOpen && <aside id={infoId} className="preview-details" aria-label="File information">
        ...
        <div className="preview-info-grid">
          ...
          <section className="preview-info-card"><h3><Clock3/>Timestamps</h3><dl><div><dt>Captured</dt><dd>{formatCapturedAt(file.capturedAtMs)}</dd></div><div><dt>Modified</dt><dd>{file.time}</dd></div><div><dt>Imported</dt><dd>{file.time}</dd></div></dl></section>
