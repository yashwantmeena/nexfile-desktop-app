...
export function FileCard({ file, selected, favorite, onSelect, onFavorite }:FileCardProps) {
  return <article className={`file-card${selected?" selected":""}`} onClick={onSelect}><div className="file-card-top"><button className="select-dot" aria-label="Select file"><i/></button><button className="file-menu" aria-label="File menu"><MoreHorizontal /></button>{file.image&&<button aria-label={favorite?"Remove from favorites":"Add to favorites"} className={`card-star${favorite?" favorite":""}`} onClick={(event)=>{event.stopPropagation();onFavorite();}}><Star /></button>}</div><FileArtwork file={file}/><div className="file-copy"><strong>{file.name}</strong><span>{file.path}</span><small>Modified {file.time}</small></div></article>;
}
