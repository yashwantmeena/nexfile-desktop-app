...
interface FileCardProps { file:DashboardFile; previewUrl?:string; selectable:boolean; selected:boolean; onToggleSelection:()=>void; onOpen:()=>void; }
...
export function FileCard({ file, previewUrl, selectable, selected, onToggleSelection, onOpen }:FileCardProps) {
  const [previewFailed, setPreviewFailed] = useState(false);
  useEffect(() => setPreviewFailed(false), [previewUrl]);
  const showPreview = Boolean(previewUrl) && !previewFailed;
  return <article className={`file-card visual-file-card ${showPreview?"visual-only-file-card":"named-file-card"}${selected?" selected":""}`} role="button" tabIndex={0} aria-label={`Preview ${file.name}`} onClick={onOpen} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();onOpen();}}}>
    {selectable && <button type="button" className="visual-card-selection" aria-label={selected?`Deselect ${file.name}`:`Select ${file.name}`} aria-pressed={selected} onClick={event=>{event.stopPropagation();onToggleSelection();}} onKeyDown={event=>event.stopPropagation()} />}
