    ...
    <section className="drive-panel">
      ...
      <div className="drive-table-scroll"><div className="drive-table">
        ...
        {drives.map((drive,index) => <div className="drive-row" key={drive.id}>
          ...
          <div className="limit-cell"><label className="limit-input"><input type="number" min="0" step="1" defaultValue={drive.appLimitValue} placeholder="No limit" aria-label={`${drive.driveName} ${drive.partitionName} storage limit`} /><span>{drive.appLimitUnit}</span>{drive.status === "mounted" && <><b>/</b><em>{drive.total}</em></>}</label></div>
