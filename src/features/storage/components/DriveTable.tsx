    ...
    <section className="drive-panel">
      ...
      <div className="drive-table-scroll"><div className="drive-table">
        ...
        {drives.map((drive,index) => <div className="drive-row" key={`${drive.driveId || drive.partitionName}-${index}`}>
          ...
          <div className="limit-cell"><label className="limit-input"><input type="number" min="0" step="1" defaultValue={bytesToGigabytes(drive.appLimitBytes)} placeholder="No limit" aria-label={`${drive.driveName} ${drive.partitionName} storage limit in GB`} /><span>GB</span>{drive.isMounted && <><b>/</b><em>{formatCapacity(drive.totalBytes)}</em></>}</label></div>
          ...
          <div className="drive-actions">{drive.isMounted ? <button className="unmount-action" onClick={()=>onUnmount?.(drive.driveId)}>Unmount</button> : drive.isConnected ? <button onClick={()=>onMount?.(drive.driveId)}>Mount</button> : <span className="unavailable-label">Unavailable</span>}</div>
        </div>)}
      </div></div>
    </section>
  );
