...
export function StoragePage({ activeNavigation, onNavigationChange }: StoragePageProps) {
  ...
  const mountDrive = (id:string) => { const drive=unmountedDrives.find(item=>item.driveId===id); if(!drive)return; setUnmountedDrives(items=>items.filter(item=>item.driveId!==id)); setMountedDrives(items=>[...items,{...drive,priority:items.length+1,status:"mounted",available:drive.totalCapacity}]); setHasUnsavedChanges(true); };
  const unmountDrive = (id:string) => { const drive=mountedDrives.find(item=>item.driveId===id); if(!drive)return; setMountedDrives(items=>items.filter(item=>item.driveId!==id).map((item,index)=>({...item,priority:index+1}))); setUnmountedDrives(items=>[...items,{...drive,status:"unmounted"}]); setHasUnsavedChanges(true); };
  const movePriority = (id:string,direction:"up"|"down") => { setMountedDrives(items=>{ const currentIndex=items.findIndex(item=>item.driveId===id); const targetIndex=direction==="up"?currentIndex-1:currentIndex+1; if(currentIndex<0||targetIndex<0||targetIndex>=items.length)return items; const reordered=[...items]; [reordered[currentIndex],reordered[targetIndex]]=[reordered[targetIndex],reordered[currentIndex]]; return reordered.map((item,index)=>({...item,priority:index+1})); }); setHasUnsavedChanges(true); };
